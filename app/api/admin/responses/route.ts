  ...
  return createZip([
    ...
    },
    {
      ...
      content:
        ...
        '<sheets><sheet name="Ответы" sheetId="1" r:id="rId1"/></sheets>' +
        '</workbook>'
    },
    {
  ...
  try {
    ...
    if (format === "xlsx") {
      ...
      const rows = [
        header,
        ...result.rows.map((row) => {
          const guest = row.guests as { display_name?: string; salutation?: string | null; token?: string } | null;

          return [
            guest?.display_name ?? "Общая ссылка",
            guest?.salutation ?? "",
            statusLabels[row.status] ?? row.status,
            row.plus_one ? "Да" : "Нет",
            row.entered_names,
            guest?.token ? `/invite/${guest.token}` : "",
      ...
      return new NextResponse(workbook, {
        headers: {
          ...
          "content-disposition": `attachment; filename="wedding-responses.xlsx"`
        }
      });
    }
